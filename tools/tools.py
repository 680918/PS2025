...
def get_memory(memory_type="skill", topic=None):
    ...
    return get_memory_context(memory_type)


def create_learning_plan(user_profile, skill_map=None):
    ...
    if skill_map is None:
        ...
        reason = (
            "当前缺少完整的能力地图，因此先根据用户画像和长期目标生成基础学习计划。"
        )

        learning_plan = {
            "month_1": {
                "focus": "Python基础实践",
                "topics": ["函数", "字典", "模块", "文件读写", "异常处理"],
                "milestone": "完成一个可运行的小型Python程序",
            },
            "month_2": {
                "focus": "Agent基础组件",
                "topics": ["Controller", "State", "Tool", "Registry"],
                "milestone": "理解并运行一个最小Agent闭环",
            },
            "month_3": {
                "focus": "Agent小项目",
                "topics": ["Planner", "Executor", "Memory", "测试"],
                "milestone": "完成一个基础Agent Demo",
            },
        }
    ...
    if python_level < 30:
        ...
        learning_plan = {
            "month_1": {
                ...
                "milestone": "完成一个包含多个函数和文件读写的小型python程序",
            },
            "month_2": {
                "focus": "python组件代码化",
                "topics": [
                ...
                "milestone": "独立完成一个最小Agent执行闭环",
            },
            "month_3": {
                "focus": "Agent项目实践",
                "topics": ["Planner", "Executor", "Memory", "错误处理", "测试"],
                "milestone": "完成一个可以持续迭代的Agent Demo",
            },
        }
    else:
        ...
        reason = (
            f"当前python能力为{python_level}分，已经具备进一步进行Agent项目实践的基础。"
        )

        learning_plan = {
            "month_1": {
                "focus": "Agent核心模块独立实现",
                "topics": ["Controller", "State", "Tool", "Registry", "Executor"],
                "milestone": "独立完成一个最小Agent执行闭环",
            },
            "month_2": {
                "focus": "Agent能力扩展",
                ...
                "milestone": "完成具备规划、记忆和评估能力的Agent",
            },
            "month_3": {
                "focus": "完整Agent项目",
                "topics": ["项目架构", "模块集成", "测试", "日志", "迭代优化"],
                "milestone": "完成一个可持续迭代的Agent Demo",
            },
        }
...
def load_tool_schemas():
    ...
    schema_path = os.path.join("tools", "tool_schemas.json")
...
def filter_tool_schemas(tool_names):
    ...
    return [tool for tool in schemas if tool["name"] in tool_names]
