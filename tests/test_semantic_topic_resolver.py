...
class FakeEmbeddingProvider:
    def embed(self, text):
        vectors = {
            "怎样把重复的代码整理成一个可以反复调用的模块？": [1.0, 0.0, 0.0],
...
def test_ambiguous_match_should_be_rejected():
    class AmbiguousEmbeddingProvider:
        def embed(self, text):
            vectors = {
                "这是一个模糊的问题": [1.0, 1.0, 0.0],
                "封装一段可以重复使用的代码逻辑": [1.0, 0.9, 0.0],
                "把一段逻辑组织起来供以后调用": [1.0, 0.9, 0.0],
                "定义一个可以多次调用的代码单元": [1.0, 0.9, 0.0],
                "保存程序运行过程中的数据": [0.9, 1.0, 0.0],
                "存储计算产生的值供后续使用": [0.9, 1.0, 0.0],
                "给数据或计算结果一个名字并保存": [0.9, 1.0, 0.0],
                "重复执行同一个操作": [0.0, 0.0, 1.0],
                "让一段代码连续运行多次": [0.0, 0.0, 1.0],
                "对相同任务进行多次重复执行": [0.0, 0.0, 1.0],
            }
