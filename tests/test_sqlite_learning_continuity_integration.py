...
def test_day2_should_restore_day1_state_from_sqlite(
    tmp_path,
    monkeypatch,
):
    database_path = tmp_path / "learning_sessions.db"

    day1_repository = SQLiteLearningSessionRepository(database_path)

    create_and_save_learning_session(
        repository=day1_repository,
    ...
    day2_repository = SQLiteLearningSessionRepository(database_path)
    ...
    state, response = run_agent_runtime(
        ...
        journey_id="journey_001",
        learning_session_repository=day2_repository,
    )
