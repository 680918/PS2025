...
from agent.controller import (decide_failure_action, run_planning_workflow, run_planning_runtime)
...
def test_run_planning_workflow_failure(monkeypatch):
    ...
    assert (
        ...
        ["skill_map"]
        ["error_type"]
        == "temporary_read_error"
    )

    assert (
        "create_learning_plan"
        not in data["tool_results"]
    )
...
def test_runtime_retry_fail_replan_degraded_success(monkeypatch):
    ...
    state, runtime_status = run_planning_runtime(
        "帮我制定未来三个月AI Agent学习计划"
    )
...
@pytest.mark.workflow
def test_runtime_retry_success_resume_with_mock(
    monkeypatch
):

    original_execute_tool = (
        executor_module.execute_tool
    )
...
@pytest.mark.workflow
def test_direct_replan_then_retryable_error_should_stop(
    monkeypatch
):
    ...
    assert (
        call_count["create_learning_plan"]
        == 2
    )
...
@pytest.mark.workflow
def test_run_planning_agent_success_output(
    monkeypatch
):
...
@pytest.mark.workflow
def test_run_planning_agent_stop_output(
    monkeypatch
):
